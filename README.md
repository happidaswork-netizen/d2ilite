...
TITI 机器元数据当前唯一主规范见 `docs/TITI元数据规范_v2.md`。D2I Lite 负责单图读取、编辑、抓取补全和轻量动作，写入 `titi-meta` 时应兼容历史 v1，并以 v2 作为新写入口径。

## 当前图片动作线规划

后续 D2I Lite 将维护一条“当前图片动作线”：面向单张图片的随手处理能力，例如加名字白条、AI 修复、AI 单图生图、AI 元数据补全。TITI 仍负责批量生成与批量编排，Quicker/右键只作为入口层传入当前文件路径。

接手开发前请先读：

- `docs/当前图片动作线接手说明_2026-06-26.md`
- `docs/加名字白条功能规格_2026-06-26.md`
- `docs/AI单图动作接口规划_2026-06-26.md`
...
- 成品图片默认按姓名重命名并直接输出到你选择的输出目录（不再额外创建 `downloads/named/`）
- GUI 可直接“公共抓取(通用)”：输入链接后可按次设置抓取间隔/超时/JSL，并在状态栏实时看到已抓取条数
- 可开启“仅保留最终图片 + 抓取记录文档”：完成后自动清理中间文件；`crawl_record.json` 内保留模板快照与抓取轨迹，便于回溯
- 抓取设置支持模板选择：可直接选已有模板，或按当前链接自动生成模板并一键开始任务（可选任务后自动清理模板文件）
- 选择已有模板时，系统直接使用模板内 `start_urls` 启动任务，无需再次手工输入链接
...
- `gender` 未识别时保持空值，避免写入 `unknown`/`unkonw` 占位词；只在页面文字明确时映射，不做人脸推断
