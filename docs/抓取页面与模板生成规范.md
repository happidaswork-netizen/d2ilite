...
> **D2I Cloud 终落点合同（强制补充）**  
> 凡进入 Cloud 队列、并会 `finalize` 到 **角色肖像** / **people.sqlite** 的模版，必须同时满足  
> [`d2i_cloud_template_extract_contract.md`](d2i_cloud_template_extract_contract.md)：  
> **Must** = 合格 `name` + `detail_url`；行政区/`unit_name` 写在 rules；禁止导航名与宽 list；有图只取人像。  
> 与本文表述冲突时，**Cloud 抽取合同优先**。
...
- 公开档案/新闻稿/英烈墙类模板默认按 D2I/TITI 元数据规范写入:
  - 每张最终图片都写入 `Xmp.dc.description` 和 `EXIF XPComment`，内容优先为详情页完整原文。
  - 同一份完整原文也写入 `Xmp.titi.meta > d2i_profile.description/full_content`。
  - 多图文章必须为每张图保留同一篇原文；当前图片 URL 写入 `d2i_profile.image_url`，资料图序号写入 `image_role/image_index/image_total`。
  - `metadata_key` 仅作为处理/恢复键，放在 `d2i_profile` 和 JSONL 记录中；不得新增 `titi:meta` 顶层强制字段。
  - `mapped_fields/extra_fields` 内的重复长文本可压缩或省略；短文完整原文由 `description/full_content` 承载，不得截断。
  - 当完整原文超过 JPEG XMP/EXIF 单段容量时，完整原文写入图片内部 JPEG COM 分块，前缀为 `D2I-FULL-CONTENT-V1`；`description/full_content` 可写预览和定位信息，并通过 `full_content_storage/full_content_sha256/full_content_chars/full_content_preview` 标明读取方式。
  - sidecar 记录（如 `crawl_record.json`）可作为备份，但不能替代图片内嵌元数据。
