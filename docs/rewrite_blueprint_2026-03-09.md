...
1. 规格基线与验证资产：已就绪
2. 新版主工程骨架：已就绪
3. 新版正式产品结构：已完成第一轮拆分与稳定基线整理，但长期 `domain / infrastructure` 收敛尚未开始
...
2. 然后把 `features/metadata/model.ts` 中的模型、格式化、批量规则下沉到 `domain/`。
3. 再把 bridge / preview / 本地 I/O 收敛到 `infrastructure/`。
