...
3. **运行层轻量优先**：搜集层可以全量完整；队列、状态接口、最近生成结果和普通生成图的热路径只读取轻量索引。
...
真人图片与其关联新闻、网页、采访、百科、社媒公开资料，是 TITI 的核心资产之一。TITI 的正式口径是：**能抓到的资料必须全量保留、全量入库、全量可追溯，并且能够随角色卡一起导出为完整资料包**。

这里的分层不是为了删减资料，而是为了保证资料不会丢、不会混、不会因为塞错字段导致前端和酒馆变慢：

1. 原始资料进入 `research_sources` / `research_documents` / 本地归档文件。
2. 结构化事实进入 `key_facts` / `timeline_events` / `persona_reference`。
3. 酒馆主卡字段进入经过整理的可扮演摘要。
4. 酒馆导出包可以同时包含完整 `extensions.titi.research_sources`、`character_book` 条目和独立 JSON 资料包。

### 8.1 资料证据层

外部资料必须存入 `research_sources`，作为可追溯资料。TITI 不应只保存摘要和链接；如果抓取流程拿到了正文、截图、网页快照、图片来源页或人工备注，应保留完整资料引用和本地归档路径。
...
| --- | --- |
| `url` | 能保留则必须保留，方便回查 |
| `raw_text` / `raw_text_ref` | 能保留则必须保留；长正文优先放 DB/sidecar/归档文件 |
...
| `use_scopes` | 可用于哪些下游：`local_archive` / `export_bundle` / `prompt_context` / `tavern_book` / `tavern_profile_draft` |
| `source_note` | 记录来源、抓取方式和归档说明 |
| `confidence` | `low` / `medium` / `high` |

### 8.2 角色表现层

酒馆卡主字段使用整理后的角色表现资料；但完整原始资料仍随 TITI 资料包进入导出结果。也就是说，`description/personality/scenario` 是“可聊天摘要”，`extensions.titi.research_sources` 和配套 JSON 是“完整证据与资料库”。
...
5. 生图、AI 改写、酒馆导出都可以读取这些真人资料，但必须保留来源链路，避免事实和创作加工混在一起。

### 8.3 真人资料全量保全规则

如果资产对应真实人物：

1. 抓取到的新闻、网页、正文、截图、来源页、图片来源、人工备注都要进入 TITI 资料层。
2. 图片文件内嵌层只放短索引、短摘要和来源引用；完整资料放 DB/sidecar/归档文件。
3. 酒馆 PNG/JSON 主卡可以保持轻量，但导出包必须能带上完整 research bundle。
4. 事实资料和创作加工都要保留，但字段分开：事实在 `research_sources`，可写作抽象在 `persona_reference`，聊天人格在 `tavern_profile`。
5. 如果用于小说/虚构角色，应在 `persona_reference.basis` 标记为 `fictionalized_from_public_sources` 或 `fictional_character_inspired_by_sources`，同时继续保留原始资料链路。

### 8.4 酒馆导出资料包

导出酒馆角色卡时，TITI 应支持三种粒度：

| 模式 | 内容 |
| --- | --- |
| `card_only` | 只导出酒馆主卡字段和 TITI 来源引用 |
...
```json
{
  "tavern_export_policy": {
    "mode": "full_research_bundle",
    "include_raw_sources": true,
    "include_character_book": true,
    "include_archive_manifest": true
  }
}
```

一句话：**真人新闻资料在 TITI 中必须全量保留、全量入库、全量可追溯；酒馆主卡吃整理后的角色表达，导出包带完整资料链路。**
...
因此，TITI 的搜集层仍然全量保留资料；JPG 内嵌层只承担“可携带索引”的职责：

1. JPG 内嵌层放轻量可携带字段：`titi_asset_id`、`titi_world_id`、`titi_content_hash`、短 `identity_prompt`、短摘要、来源 URL、`research_source_refs`。
2. 新闻全文、采访全文、长角色卡、世界书、完整提示词迭代记录放 DB / sidecar / 归档文件 / 导出 JSON。
3. 图片里通过 `source_ids` 或 `research_source_refs` 指向完整资料，不以图片内嵌层作为唯一存储。
...
1. 搜集层、资料层、归档层可以全量完整保存。
2. 前台热路径不扫描大图库来补全资料。
3. 队列状态和 WebSocket 不读取完整资料包。
4. 最近生成结果只读 TITI 自己生成的短期记录。
5. 普通生成图的内嵌 `titi:meta` 保持轻量索引。
6. `tavern_profile`、长示例对话、世界书、完整原文、新闻全文等大字段以 DB/sidecar/归档文件为权威。
...
- 新增外部资料区：`research_sources` / `research_documents` / `persona_reference`
...
- 资产页增加酒馆资料编辑与预览。
- 资产页增加资料来源管理、全文归档、摘要提取、来源链路和导出范围标记。
